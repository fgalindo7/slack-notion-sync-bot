...
export function loadConfig() {
  ...
  const config = {
    ...
    // AI Suggestions configuration (Vertex AI)
    vertexAI: {
      enabled: String(process.env.AI_SUGGESTIONS_ENABLED || 'false').toLowerCase() === 'true',
      projectId: process.env.GCP_PROJECT_ID || '',
      location: process.env.VERTEX_AI_LOCATION || 'us-central1',
      model: process.env.VERTEX_AI_MODEL || 'gemini-pro',
      maxHistoricalCases: parseInt(process.env.AI_MAX_HISTORICAL_CASES || '20', 10),
      similarityThreshold: parseFloat(process.env.AI_SIMILARITY_THRESHOLD || '0.7'),
      queryDaysBack: parseInt(process.env.AI_QUERY_DAYS_BACK || '90', 10),
      cacheTTL: parseInt(process.env.AI_CACHE_TTL || '300000', 10) // 5 minutes
    },
  ...
  if (config.vertexAI.enabled) {
    if (!config.vertexAI.projectId) {
      logger.warn('AI suggestions enabled but GCP_PROJECT_ID not set, disabling feature');
      config.vertexAI.enabled = false;
    }
    
    if (config.vertexAI.similarityThreshold < 0 || config.vertexAI.similarityThreshold > 1) {
      logger.warn({ 
        value: config.vertexAI.similarityThreshold 
      }, 'AI_SIMILARITY_THRESHOLD out of range (0-1), using 0.7');
      config.vertexAI.similarityThreshold = 0.7;
    }
    ...
    if (config.vertexAI.enabled) {
      logger.info({
        ...
        similarityThreshold: config.vertexAI.similarityThreshold
